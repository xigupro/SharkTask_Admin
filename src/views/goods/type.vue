<template>
  <div
    class="xg-main goods-type-page"
    v-loading="isListLoading">
    <div class="main-header">
      <el-button
        v-if="$tools.checkAdminPower('goods', 'addType')"
        type="primary"
          ...
          <template slot-scope="scope">
              <el-dropdown
                ...
                <el-dropdown-menu
                  ...
                  <el-dropdown-item
                    v-if="$tools.checkAdminPower('goods', 'editType')"
                  ...
                  <el-dropdown-item
                    v-if="$tools.checkAdminPower('goods', 'deleteType')"
                    class="xg-popover-list__item"
...
export default {
  ...
  methods: {
    ...
    async submit() {
      ...
      const res = await GoodsManageProxy[this.isEdit ? 'updateType' : 'addType'](this.form);
      if (!res.fail) {
        this.$notify.success({
          title: '成功',
          message: res.message,
    ...
    async removeType(id) {
      ...
      const res = await GoodsManageProxy.removeType(params);
      if (!res.fail) {
        this.$notify.success({
          title: '成功',
          message: res.message,
